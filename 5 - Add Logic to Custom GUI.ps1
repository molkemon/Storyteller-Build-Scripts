...
Write-Host "Adding Custom GUI logic (Synchronized Mode)..." -ForegroundColor Cyan

$eventCounter = 1
# SORTING IS CRITICAL: Must match Script 4's sorting exactly
...
$bulkInsert = New-Object System.Collections.Generic.List[string]

foreach ($eventEntry in $csvData) {
    $descKey = $eventEntry.eventId
    
    # Skip the settings menu key to maintain sync with Script 4
    if ($descKey -eq "anb_settings.1.d") {
        $eventCounter++
        continue
    }

    $bulkInsert.Add("        else_if = { limit = { check_variable = { which = ${modName}_id value = $eventCounter } } play_sound = $descKey }")
    $eventCounter++
}

$bulkInsert.Reverse()
if ($bulkInsert.Count -gt 0) { $bulkInsert[0] = $bulkInsert[0] -replace "else_if", "if" }
$bulkInsertBlock = $bulkInsert -join "`n"

$templateFile = [System.IO.Path]::Combine($scriptFolder, "bootstrap", "common", "custom_gui", "storyteller.txt")

# FIXED SYNTAX: Removing ForEach-Object and using parentheses to properly chain -replace and -split
[string[]]$customGuiContent = if (Test-Path $templateFile) { 
    ((Get-Content $templateFile -Raw -Encoding UTF8) -replace '\$modName', $modName) -split "`r?`n" 
} else { 
    Get-Content $customGuiFile 
}

$updatedGuiContent = New-Object System.Collections.Generic.List[string]
$contentString = $customGuiContent -join "`n"

# Look for an existing play_sound call that isn't a placeholder
if ($contentString -match "play_sound\s*=\s*[^$\s\{\}]+") {
    Write-Host "GUI file already contains logic. Skipping." -ForegroundColor Red
} else {
    $inserted = $false
    foreach ($line in $customGuiContent) {
        $updatedGuiContent.Add($line)
        if (-not $inserted -and $line -match "^\s*effect\s*=\s*\{") {
            $updatedGuiContent.Add($bulkInsertBlock)
            $inserted = $true
        }
    }
    if (!(Test-Path $customGuiFolder)) { New-Item -ItemType Directory -Force -Path $customGuiFolder | Out-Null }
    $updatedGuiContent | Set-Content -Path $customGuiFile
    Write-Host "Successfully synced $($bulkInsert.Count) GUI IDs." -ForegroundColor Green
}